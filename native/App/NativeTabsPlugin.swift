        ...
        if let i = args.firstIndex(of: "-smokeTab"), i + 1 < args.count {
            ...
            if action == "openSignup" {   /* ⑪ Turnstile: 가입 화면 진입 → +6s 토큰·iframe 상태를 JSON에 기록 */
                DispatchQueue.main.asyncAfter(deadline: .now() + 3.0) { [weak self] in
                    self?.bridge?.webView?.evaluateJavaScript("(function(){try{var d=document.getElementById('dmAgree'); if(d) d.click();}catch(e){} try{ if(!window.__fw){ window.__fw=window.fetch; window.fetch=function(u,o){ try{ if(o&&o.headers&&o.headers['X-CampusDoIt-App']) window.__hdrSent=true; }catch(e){} return window.__fw.apply(this,arguments); }; } goSignup(); setTimeout(function(){ try{ var i=document.getElementById('suId'); i.value='smokeuser1'; checkDup('id'); }catch(e){} },800); setTimeout(function(){ try{ var t0=Date.now(); fetch(API_BASE+'/api/v1/auth/check?login_id=test111',{headers:{'X-CampusDoIt-App':(typeof APP_HDR_SECRET!=='undefined'?APP_HDR_SECRET:'')}}).then(function(r){ return r.text().then(function(b){ window.__apiCheck={status:r.status,ms:Date.now()-t0,body:b.slice(0,160),url:API_BASE+'/api/v1/auth/check'}; }); }).catch(function(e){ window.__apiCheck={status:0,err:String(e),url:API_BASE+'/api/v1/auth/check'}; }); }catch(e){ window.__apiCheck={status:-1,err:String(e)}; } },2500); }catch(e){window.__smokeErr=String(e);} return 1;})()", completionHandler: nil)
                }
            }
